...
また、この二つの手法を併用すると、多くのデメリットを補うこともできます(詳細は2.1、2.2を参照)。<br/>
...
前節で見たように、二つのデカールの処理を併用することで、多くのデメリットを補完できます。<br>
ここでは併用の仕方として、次のモデルケースとして提示します。
...
一方、URPデカールはメッシュ生成を行う必要はありませんが、デカール表示のためのピクセルシェーダーで、複数回の行列演算とテクスチャサンプリングが追加が毎フレーム必要になります。そのためシンプルなメッシュ描画に比べると描画パフォーマンスが悪化します。<br/>
