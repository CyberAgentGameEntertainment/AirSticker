...
The following parameters can be set for the AirStickerProjector component.
...
|Group ID|The group ID of the decal mesh.<br/>Decal meshes are shared (merged into one mesh) only within the same group. By splitting groups, decals can be removed by group using the AirStickerProjector.RemoveDecalMeshes() method (see 4.4).<br/>The more groups you use, the more draw calls are made. So if you don't need to remove decals separately, leave it 0.|
...
### 4.4 How to remove decals by group
Decal meshes generated by Air Sticker are shared (merged into one mesh) when the receiver object, the renderer, the decal material and the group ID are the same.<br/>
By default, all decals belong to the group 0, so they are merged as much as possible to reduce draw calls.<br/><br/>
If you want to remove some decals separately, specify a different group ID using the groupId argument of the CreateAndLaunch() method (or the Group ID parameter of the AirStickerProjector component).<br/>
Decals that belong to different groups are built as separate decal meshes, so the more groups you use, the more draw calls are made. It is recommended to split groups only for the decals you want to remove separately.<br/><br/>
Decals can be removed by group using the AirStickerProjector.RemoveDecalMeshes() method.<br/>
```C#
// Paste a sticker as the group 1.
AirStickerProjector.CreateAndLaunch(
                projectorObj,
                receiverObject,
                decalMaterial,
...
// Remove all decal meshes that belong to the group 1.
AirStickerProjector.RemoveDecalMeshes(1);

// The receiver object and the decal material can also be specified
// to narrow down the removal target.
AirStickerProjector.RemoveDecalMeshes(1, receiverObject, decalMaterial);
```
Note that the RemoveDecalMeshes() method should be called after the projectors of the target group have finished launching (LaunchingCompleted).<br/>
